  ...
  import {
	...
	function addmenu(row){
		if (row) {
			...
			olddata.push(
				{id : rows.length + 1 ,tableid : tableselect.tableid , productid : row.pcode ,productname : row.pname , price : row.price , send : '0' ,payed : '0'}
			)
			rows = {}
...
<Content>
	<Grid>
		...
		<Row>
			<Column>
				<Row>
					<Column md={4} lg={4} style="background:white;">
						<Dropdown
						titleText="โต๊ะ"
						bind:selectedId ={tableselect.tableid}
						items={tables}
						/>			
						<br>
				...
				<Row>
					...
					<Column md={4} lg={4}>
						<Button on:click={addmenu(item)}>{item.pname}</Button>
					</Column>
					{/each}				
				</Row>
			</Column>
